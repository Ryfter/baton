...
try {
    ...
    $testPath = Join-Path $HOME 'dev' 'Whittle'
    Write-SessionMarker -Agent 'claude' -SessionId 'sess-1' -Cwd $testPath -BatonHome $tmp
    $act = @(Get-ActiveSessions -BatonHome $tmp)
    Assert 'M1 one active marker after write' (@($act).Count -eq 1)
    Assert 'M2 marker carries agent+cwd' ($act[0].agent -eq 'claude' -and $act[0].cwd -eq $testPath)
    Assert 'M3 Test-FolderActive matches case-insensitively' (Test-FolderActive -Folder (Join-Path $HOME 'dev' 'whittle') -Sessions $act)
    Assert 'M4 Test-FolderActive no false match' (-not (Test-FolderActive -Folder (Join-Path $HOME 'dev' 'Other') -Sessions $act))
    ...
    $stale = @{ agent='claude'; session_id='old'; cwd=(Join-Path $HOME 'dev' 'Old'); started_at=(Get-Date).AddHours(-48).ToString('o') }
