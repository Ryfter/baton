...
def find_script_root() -> Path:
    """Directory containing context_packet.py (same folder as this script)."""
    here = Path(__file__).resolve().parent
    if (here / "context_packet.py").is_file():
        return here
    raise RuntimeError(
        "Could not find context_packet.py next to select_context.py"
    )
