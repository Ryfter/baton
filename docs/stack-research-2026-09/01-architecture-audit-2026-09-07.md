# Baton architecture audit — 2026-09-07

**Two-pass fleet audit.**
Pass 1: **GLM 5.3 Flash** (`z-ai/glm-5.3-flash`, OpenRouter) — $0.0025, clean stop, 17.3 KB.
Pass 2 (adversarial): intended for **grok-cli**. Grok headless failed 3× tonight —
two `cli-chat-proxy.grok.com` stream errors mid-generation, plus its `--prompt-file`
mode offloads a large prompt and exits on a stub. `pgrep` showed the cause: a
scheduled `grok --always-approve` gauntlet job is running and holds `~/.grok/leader.sock`.
So pass 2 was done by **Claude (Sonnet)** against the live repo. Every LOC figure
GLM cited was verified against `scripts-inventory.txt` first — **all accurate, zero
fabrication.** A later grok retry (gauntlet job cleared) produced only a ~950-char
partial before the same headless failure — that fragment **corroborates** the Claude
corrections below (see Part C). grok headless failed 4× total tonight across 4 output
modes: this is #183/#197 reproducing, hard.

Inputs: `scratchpad/audit/{brief,bundle}.md` + inventories (scripts **36,120 code /
23,686 test LOC**; 125 py files incl. `dashboard/` 8,721 + `kb/` 1,734; 32 open issues).

---

## Bottom line (both passes agree)

The owner's instinct is right and understated. Baton is **four mechanisms doing one
job** (Claude Code plugin + MCP server + 60 kLOC pwsh + `fleet.yaml`), on a runtime
(`pwsh`) that intermittently SIGABRTs on this Mac, and **~two-thirds of the pwsh body
is either someone else's tool or ceremony that was never the goal.**

Don't split Baton into several apps. **Collapse it into one owned CLI (~5–6 kLOC
Python) and adopt five external tools:** firstmate (crew + worktrees), treehouse
(worktree pool), quota-axi (quota probe), no-mistakes (validation gate), gnhf
(overnight-loop semantics). The MCP server dies. The plugin shrinks to one slash
command + the Python hooks.

**The single unresolved architecture decision — neither pass answers it:** adopting
firstmate is not "delete a lib, call a CLI." firstmate is an *agent distro* (its own
`AGENTS.md`, session backends, liaison agent). Does `baton go` shell out to
firstmate, or does firstmate's liaison call `baton`? Who owns the loop? Decide that
before any code.

---

## PART A — Claude adversarial pass (corrections to GLM)

GLM's facts are clean and its shape is right. Six judgment calls need fixing, one
omission is material, and the strangler plan should be kept verbatim.

### A1. `officers-lib.ps1` (1,574) — GLM said DROP "persona roleplay". **Wrong.**
Functions: `Get-OfficerRegistry`, `Read/Write-SchedulerState`,
`Record-SchedulerFableFire`, `Test-JobWantsFable`, `Test-JobExcessCapacity`,
`Get-SchedulerWindowSnapshot`. This is the **Fable scheduler / layer-seating (d124)
enforcement** — when the plan + sprint-review layer fires, gated on capacity and the
usage window. Not roleplay. → **CORE** (trimmable, but it's scheduling policy, not a
1.5 kLOC delete).

### A2. `gate-lib.ps1` (598) — GLM said → no-mistakes. **Partly wrong.**
Functions: `Get-AcceptanceVerdict`, `Merge-ReviewFindings`, `Invoke-AcceptanceGate`,
`Build-ReviewPrompt`, `Get-ReviewRoles`. This is Baton's **competitive named-panel
acceptance review** (`/baton:gate`, `/baton:plan-gate`) — multi-role accept/polish/
reject + polish brief (d086 spine, #115). no-mistakes is a git-proxy test/lint/review
pipeline — it can be the *mechanical* gate, but the competitive panel + polish brief
is a Baton design choice. → **split: mechanical gate = ADOPT (no-mistakes); the
panel verdict logic = CORE (~250 LOC).**

### A3. `verification-lib.ps1` (724) — GLM said → no-mistakes. **Mostly right, one carve-out.**
`Get-VerificationContract` / `Get-FrozenVerificationContract` — the *contract* (what
evidence a task must produce to graduate, #96) is CORE config. Adopt the runner,
keep the contract (~120 LOC).

### A4. `coordination-lib.ps1` (915) — GLM said → firstmate. **Partly wrong — contradicts its own issue map.**
Functions: `Get-CoordClaimPath`, `Get-CoordResourceLockKey`, `Write-CoordFileAtomic`,
`Write-CoordJournalEntry`, `Get-CoordProcessInfo` — resource-claim locking +
coordination journal + process liveness. That **is** #211 (concurrent-session git
guard) and #202 (human boundary from process state), which GLM itself puts in CORE.
→ **claim/lock/liveness primitives = CORE (~300 LOC); crew fan-out = ADOPT (firstmate).**

### A5. `dark-factory-lib.ps1` (522) — GLM said → treehouse. **Wrong tool.**
Functions: `Get-DarkFactoryLanes`, `New-DarkFactoryMaestroJob`, `Invoke-DarkFactorySeed`,
`Get-DarkFactoryStatus`, `Get-DarkFactoryProjectBrief`, `Get-DarkFactoryStandingOrderText`.
This is the **overnight autonomous multi-project lane runner** — that's **gnhf**
(overnight loop), not treehouse (worktree pool). Lane/standing-order/project-brief
config → **CORE**; the loop mechanics → **gnhf semantics**.

### A6. `fleet-executor-lib.ps1` (2,512) — GLM said → firstmate. **Right, and it's the biggest single win.**
Confirmed: `New-RunWorktree`, `Get-RunDiff`, `Restore-WorktreeTreeSnapshot`,
`Test-ProviderAgentic/DiffApply/EditCapable`. Worktree lifecycle + provider-capability
probing + diff extraction → firstmate + treehouse. **Residue:** provider-capability
truthing (#183) = ~60 LOC CORE roster config.

### A7. **Material omission: `dashboard/` — 8,721 LOC of Python, never mentioned.**
Bigger than GLM's entire CORE port estimate. It's the observability surface Kun
explicitly wants ("status surfaces that distinguish still-working from dead").
→ **KEEP, freeze.** Once the event log (#206) is the source of truth, the dashboard
is a read-only projection client — low risk. Do not expand it. Don't port it; it's
already Python.

### A8. Revised bucket totals
| Bucket | GLM | Claude-revised | Note |
|---|--:|--:|---|
| CORE (pwsh → Python) | ~12,000 | **~13,500** | +officers scheduler, +coord/dark-factory config, +gate panel, +verify contract |
| ADOPT (delete via 5 tools) | ~9,400 | **~8,700** | −the CORE carve-outs above |
| DROP (delete outright) | ~14,700 | **~13,900** | still the largest bucket |
| **Python port target** | 4–5 kLOC | **5–6 kLOC** | pwsh inflates ~2.5× |
Plus: keep `kb/` (1,734, already Python), keep+freeze `dashboard/` (8,721), port
~2,500 LOC of CORE tests to pytest.

### A9. Issue-map fixes
- **#182** (test-heartbeat clock checks) — GLM said DROP. The window-anchoring logic
  (`/baton:heartbeat`, `/baton:window-service`, the `:11` scheduling law) is
  **CORE** and separate from `heartbeat-lib` watchers. → CORE (fix or delete the 3
  checks; keep window logic).
- **#197** (grok ACP hang) — GLM said DROP "route around it". Correct, and it bit
  this very audit (grok contended + upstream errors tonight). Broaden the lesson:
  **no single provider on the golden-path critical section**, ever.
- **#208** (name 3 verbs) — DROP as a feature, keep as a 30-min doc task; the CLI
  surface below settles it.

### A10. Keep verbatim from GLM
- **The strangler plan** (its strongest section): week-1 dumb linear `baton go`
  steps 1→7 on 2 providers + firstmate + quota-axi + no-mistakes, prove **one real
  PR**, *then* port CORE libs behind the event log one per week, **freeze `scripts/`
  immediately**, no parallel-run period (the pwsh plane has never been a correct
  enough oracle to diff against).
- **The biggest risk**: 13 kLOC of CORE pwsh encodes routing/governor/conductor
  behavior that *has never run correctly end-to-end* — there is no golden master.
  Faithful porting risks a beautiful Python system that also doesn't ship code.
- **The premise answer**: "split it up" is half right; the fix is collapse-to-one +
  adopt-five, and "its own CLI application" is exactly right.

---

## PART C — grok adjudication (partial — headless failure at ~950 chars)

grok was asked to adjudicate GLM vs Claude and give decisive answers (firstmate
ownership direction, Python vs Go, build-first). It streamed a partial then died —
same offload/stream failure as the other 3 attempts. What it produced before cutting:

> **Where GLM is wrong or soft.** Wrong on files, not on LOC — the inventory numbers
> are treated as given, the buckets are not.
>
> **`officers-lib` is not “persona roleplay.”** The named functions are Fable seating /
> window gating. GLM judged the filename. That error is real. It does **not** make the
> whole 1,574 lines CORE.
>
> **`gate-lib` → no-mistakes is a category error.** no-mistakes is a git-proxy
> (test/lint/review, forward branch, open PR). Baton’s gate is a competitive
> named-panel (accept/polish/reject + polish brief). Same English word, different
> product. Mechanical gate adopts; panel does not.
>
> **`verification-lib` → no-mistakes is half a thought.** The runner is someone
> else’s. The graduation contract (what evidence a task must produce) — [cut here]

**Net:** grok independently reached the same three corrections as Part A (A1/A2/A3),
which raises confidence in them. Its one added nuance — don't over-rotate
`officers-lib` wholesale to CORE, split it — is folded into A1. It never reached the
decisive §4 answers (firstmate direction, language, build-first); those remain open
for you or a working grok session.

---

## PART B — GLM 5.3 Flash, pass 1 (raw, verified accurate)
