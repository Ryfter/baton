...
**→ NEXT (Kevin's direction — nothing auto-actionable queued).** The v2 MVP (`v1.3.0`), the four-slice v1.4 line (`v1.4.0`), the learned-cost re-rank (`v1.4.1`), and now `/baton:start` slices 1 and 2 (`v1.5.0-rc.2`) are all shipped. **Two things need Kevin's word, not autonomous action:** (1) promote `v1.5.0-rc.2` to stable (same pattern as every prior rc→stable cut); (2) whether/when to resume the parked GEPA/DSPy prompt-optimization track (own brainstorm — not started). Other optional next tracks, none auto-actionable, each its own spec→plan→build: (a) `/baton:start` slice 3 (mid-stream idea injection); (b) realized-cost **`CostResolver`** (token/dollar metering to replace the `estimate` basis — the last "listed price is a lie" gap; would flip `learned_adjust`'s reach from estimate-anchored to measured); (c) **dashboard integration** of the Obsidian Command concepts (Live Fleet Ops first; System-Intelligence MODEL LEADERBOARD consumes the `--json` leaderboard + could surface the live `learned_adjust`/`why` per candidate); (d) acceptance-gate follow-ups (auto-polish loop, task-output bus + mid-DAG gate task, budget-gated reviewer spend); (e) Conductor Style-B broker + cockpit; (f) Memory Bridge M1/M2/M3. **Through-line (Price Reversal Phenomenon, arXiv 2603.23971):** listed per-token price is an unreliable proxy for realized cost — d026's cost-optimal router that learns. Effective cost now measures it (slices 1–2) AND acts on it in routing (slice 3); the remaining honesty gap is the cost *numerator* (estimate → measured), which is the `CostResolver`.
