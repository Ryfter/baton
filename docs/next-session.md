...
**→ NEXT: a FORK Kevin decides.** (a) **`/baton:go` Maestro** — the NL plan-then-execute front door Kevin introduced 2026-06-18: `/baton:go "<what I want>"` → a Maestro spawns a *planner* orchestrator that sequences Baton's commands, then spawns *executor* orchestrators to run them, reviewing between. It's the **autonomy north-star capstone** (the "stop the 1/2 prompts" + autonomous folder→repo run-loop vision); sits ON TOP of the building-block commands (triage, research-gate, projects, ensemble, job FSM, routing). Teed up as the next initiative after Sprint 4; needs its own brainstorm → spec → plan (memory `project_maestro_go_mode`). (b) **Sprint 5 — Memory Bridge** (Projectmem-style event log + Grimdex promotion; adapter pattern, d047) per the roadmap order. **Forward hook still standing:** Sprint 6 (Worker Adapter) is where `gh models run <model>` becomes a *budgeted* fleet worker — the mechanism that turns the GitHub allotment into a saturable labor pool the Usage Governor drives toward Kevin's **99.9%-monthly-utilization** target.

**Cleanup note:** Sprint 4 was built on an in-tree feature branch (`sprint-4-research-gate`, squash-merged + deleted local & remote) — no worktree this round, nothing to prune. SDD progress ledger + task briefs live under `.git/sdd/` (non-tracked, safe to ignore). Kevin's `dashboard/static/style.css` WIP remains uncommitted in the working tree (preserved across the branch round-trip + master sync, untouched by Sprint 4).
