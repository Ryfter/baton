...
**→ NEXT: Sprint 2 — Usage Governor** (worker usage registry, manual lockout + reset-ETA tracking, availability states available|limited|exhausted|cooling_down|waiting_for_reset|conserve_mode, simple 7-day forecast, route-around-exhausted). Seeds off the `usage_class` field. Needs spec → plan → build. The pre-v2 "next builds" below (cost/speed advisor, autonomous run-loop, MCP ride-along) still stand but are now slotted within/after the v2 sprint order.

**Cleanup note:** a leftover worktree dir `D:\Dev\baton-triage` is git-untracked (already `git worktree remove`d) but was locked by a process at session end — delete it when free.
