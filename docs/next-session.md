...
1. **Cost-Optimization Engine — Slice A (time-awareness).** Spec `docs/superpowers/specs/2026-06-10-cost-optimization-engine-design.md`; plan `docs/superpowers/plans/2026-06-10-cost-optimization-engine-slice-a.md` (both on master — the plan rode merge `1dad527`). 4 levers (1=routing SHIPPED; 2=time-awareness prime-peak rank gate + weekend surge; 3=draft→finish cascade; 4=multi-platform); **A→B→C order confirmed**. Slice A = `prime-hours.ps1` gate + capacity profile + wiring (routing paid-tier, backlog driver, /route), **7 TDD tasks, regression-safe opt-in gate**. Build it in its **OWN worktree-isolated session** (multiple sessions share this repo). Rank: #1 highest…#5 lowest; 0/6 reserved-undocumented.
