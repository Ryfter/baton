...
**`/baton:start` — the Conductor front porch, slice 3 (Mid-stream idea injection): SHIPPED 2026-07-01** (plugin `1.5.0-rc.2 → 1.5.0-rc.3`). Supports a days-long, idea-driven cadence by letting users dump raw thoughts mid-project. A new `--idea "<text>"` flag and a Resume-path prompt hook capture the idea. It is folded into the project `CHARTER.md` (under "Decisions & open questions"). A fast LLM classifier (`triage` capability) routes it: tactical ideas recommend a re-plan (`/baton:go`), while epic-scale pivots route to a viability debate (`/baton:idea`). `start-lib.ps1` extended with `Invoke-IdeaInjection` and `Resolve-IdeaRouting`; tested hermetically in `test-start-lib-s3.ps1`. Spec/plan: `docs/superpowers/specs/2026-07-01-baton-start-slice3-idea-injection-design.md`.


**Parked design idea — GEPA/DSPy reflective prompt optimization (2026-07-01, not spec'd):** Kevin flagged DSPy + GEPA (arXiv 2507.19457) mid-session; investigated and confirmed a strong fit, then explicitly chose to park it and finish `/baton:start` first. The pitch: Baton already has the three things GEPA needs as byproducts of existing features — execution trace (`events.jsonl`/`decisions.jsonl`), reflection (acceptance-gate `findings`/`polish_brief`, job `lessons.md`), and metric (`effective_cost`, the exact Pareto axis GEPA optimizes over). d060 closes the learning loop at the **routing** layer only (who gets the task); GEPA/DSPy would be the sibling loop one layer deeper — optimizing **prompt content** itself (the fleet dispatch prompts, triage prompt, Conductor planner prompt), using the acceptance-gate verdict as the metric instead of hand-tuning strings. Recommended pilot scope: the Conductor's planner prompt in `conductor-lib.ps1` alone, before generalizing. Full writeup: memory `project_gepa_dspy_prompt_optimization`. Resume there — don't re-research the fit — whenever this is picked up.

**→ NEXT (Kevin's direction — nothing auto-actionable queued).** The v2 MVP (`v1.3.0`), the four-slice v1.4 line (`v1.4.0`), the learned-cost re-rank (`v1.4.1`), and now `/baton:start` slices 1, 2, and 3 (`v1.5.0-rc.3`) are all shipped. **Two things need Kevin's word, not autonomous action:** (1) promote `v1.5.0-rc.3` to stable (same pattern as every prior rc→stable cut); (2) whether/when to resume the parked GEPA/DSPy prompt-optimization track (own brainstorm — not started). Other optional next tracks, none auto-actionable, each its own spec→plan→build: (b) realized-cost **`CostResolver`** (token/dollar metering to replace the `estimate` basis — the last "listed price is a lie" gap; would flip `learned_adjust`'s reach from estimate-anchored to measured); (c) **dashboard integration** of the Obsidian Command concepts (Live Fleet Ops first; System-Intelligence MODEL LEADERBOARD consumes the `--json` leaderboard + could surface the live `learned_adjust`/`why` per candidate); (d) acceptance-gate follow-ups (auto-polish loop, task-output bus + mid-DAG gate task, budget-gated reviewer spend); (e) Conductor Style-B broker + cockpit; (f) Memory Bridge M1/M2/M3. **Through-line (Price Reversal Phenomenon, arXiv 2603.23971):** listed per-token price is an unreliable proxy for realized cost — d026's cost-optimal router that learns. Effective cost now measures it (slices 1–2) AND acts on it in routing (slice 3); the remaining honesty gap is the cost *numerator* (estimate → measured), which is the `CostResolver`.
