...
| `ddbf4d3` | **Opus adversarial review fixes** — 15 defects. C-1 (rm-rf prefilter ran pre-unquote: `r\m -rf` bypassed, verified deleting a real dir — pre-existing). H-1 (`timeout -s KILL 5 rm -rf`). H-2 (`chroot --userspec=x /jail rm -rf` double-consume). H-3 (`GIT add -A` past a case-sensitive fast path). H-4 (`git push -u origin <new-branch>` skipped the risky-ext check). M-1..M-4, Lows, Nits — see commit body. |
| `59328d2` | **Grok round-2 fixes** — 4 residuals Opus missed + FP/coverage. #2/#10 (segment split was quote-blind → `git commit -m 'fix; git add -A'` false-denied; now a real char scanner splitting only on UNQUOTED operators). #1 (`sh -c 'git add -A'` / `eval` / `git submodule foreach git add -A` invisible to publish-guard → `nested_command_bodies` + recursive `_git_invocations`). #3 (`setarch ARCH rm -rf`). #4 (`chroot --x=y NEWROOT sh -c 'rm -rf'` — `=`-flags now consume nothing). #5 (`import _cmdscan` now inside the fail-open try). #6/#7/#8/#11/#12/#14 — see body. |

Probes: `test_rm_rf_guard.py` **92/92**, `test_publish_guard.py` **39/39**. Opus C-1/H-1/H-2/H-4 and Grok #1/#2/#5 E2E-verified against real repos.

**Both review passes (Opus adversarial + Grok round-2) are folded in.** Accepted, not fixed: command-substitution *contents* (`git add $(echo .)`, `R=rm; $R -rf`) — needs shell expansion, out of scope for a lossy pre-exec guard.
...
**Fleet does the labor — Slice 1 (round-trip proof) + v1.10.0 RELEASED: SHIPPED 2026-07-05** (PR #79 squash `9602c43`, release commit `798db72`, tag **`v1.10.0`** + GitHub release, plugin `1.9.0 → 1.10.0`, notes `docs/releases/2026-07-05-v1.10.0.md`; decision **d077**; spec `docs/superpowers/specs/2026-07-04-fleet-labor-slice1-roundtrip-proof-design.md`, plan `docs/superpowers/plans/2026-07-04-fleet-labor-slice1-roundtrip-proof.md`). A NEW track (Kevin 2026-07-05: "get this working with other models ASAP"; chose the fleet-does-labor sense over run-Baton-from-Codex). **The gap it closes:** the invoke plumbing (roster, `Invoke-Fleet`, `Select-Capability`, conductor DAG) was real but only proven with Claude; `fleet doctor` only checked binary-on-PATH; `Invoke-TaskViaFleet` is documented *non-destructive by construction — real execution wired via `-Spawner`* (that executor = Slice 2, NOT built here). Slice 1 proves the pipe + hardens prompts. **`fleet doctor --live`** (`fleet-probe-lib.ps1`: `Test-FleetCanary`/`Test-ProviderReachable`/`Invoke-FleetProbe`, `Start-ThreadJob` timeout guard, injectable `-Dispatcher`/`-UrlProbe`) sends a **4-challenge canary battery** to every ENABLED provider (cli+http via `Invoke-Fleet`) in one round-trip, scored k/4 (`PONG`/`42`/`PARIS`/`COLD` — only PONG's answer is in the prompt, so an **echo-back scores 1/4 and fails**); reports `live_ok | live_fail(reason) | skip` with `--timeout`/`--json`; exit 0 iff all enabled `live_ok`. Default (non-live) doctor byte-for-byte unchanged. **Prompt hardening:** clean-tail CLI templates ending in a standalone quoted `"{{prompt}}"` now dispatch via **stdin** (`Test-StdinSafe` predicate), immune to the 965-byte/quote wall; embedded-`{{prompt}}` + shell-wrapped templates stay legacy. Built subagent-driven (haiku T1/2/4/5, sonnet T3; streamlined, no per-task reviewers). Opus whole-branch review **READY TO MERGE, 0 Critical**. **Two fixes after review — one Kevin design call, one Critical the review MISSED:** (A) `c6b9764` the single `PONG` canary lived inside its own prompt (echo-back false-passed) → replaced with Kevin's 4-challenge battery; (B) `5bec3e6` **Task 3 broke every `stdin:true` provider** (github-models, box-private reviewer pairs) by routing them through `Resolve-FleetCommand -Prompt ''`, which a **mandatory `[string]` param rejects** — the suites missed it because the fixture had NO stdin:true provider. Fixed (resolve `{{model}}` inline, no empty-prompt binding) + added a `stub-stdin` fixture + round-trip asserts. **Build lesson (new PS trap, sibling of `$Input`/`$Event`/unary-comma):** a `[Parameter(Mandatory)][string]` param REJECTS the empty string `''` (treats it as missing → throws/prompts) — never pass `-Prompt ''` to satisfy it; and a fixture must cover EVERY dispatch path (the stdin:true gap hid a Critical from opus). Suites (0 FAIL): probe-lib 21, dispatch 19, fleet-lib 40, doctor 12, bootstrap 59 (+1 deploy assert), conductor 105. Deployed 2026-07-05 (`bootstrap.ps1 -Force`, marker v1.10.0); smoke green — deployed probe scores full battery `live_ok 4/4` + echo-back `live_fail 1/4`, `fleet doctor --live` real run → `live_ok 4/4` exit 0. **Upgrade note in the release:** re-run `fleet doctor --live` after upgrading (stdin promotion changed how trailing-quoted providers receive the prompt). **→ NEXT in this track = Slice 2: the `-Spawner` executor** — send the task to the chosen instrument, capture output/edits, turn it into an applied repo change (agentic tools edit in-place; chat models emit a diff Baton applies, OR file-edit tasks route only to agentic instruments — resolved in Slice 2's spec), verify via the acceptance gate, on a branch/worktree for reversibility. Its own brainstorm→spec→plan.
