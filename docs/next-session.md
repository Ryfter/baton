...
**→ NEXT (Kevin's direction — nothing auto-actionable queued).** The v2 MVP (v1.3.0) + the four-slice v1.4 line (v1.4.0) are all shipped and released; **0 open issues**, rc line closed, no release decision outstanding. The named-deferred next slice in the effective-cost track is the **confidence-gated `Select-Capability` re-rank** (bias routing by learned `eff_cost_mean` once `confidence` clears a bar — the routing payoff that turns the advisory leaderboard into a learning router; its own spec→plan→build). Other parallel tracks (all optional, none auto-actionable): the realized-cost **`CostResolver`** (token/dollar metering to replace the estimate basis); **dashboard integration** of the Obsidian Command concepts (Live Fleet Ops first); acceptance-gate follow-ups (auto-polish loop, task-output bus + mid-DAG gate task, budget-gated reviewer spend); Conductor Style-B broker + cockpit; Memory Bridge M1/M2/M3. **Original brainstorm framing (now realized by slice 1):** quality-adjusted "effective cost" — `actual_cost ÷ quality` — the acceptance gate supplies the quality signal, the worker-adapter/usage-governor supply measured actual cost. Informed by *The Price Reversal Phenomenon* (arXiv 2603.23971, PDF in Kevin's Downloads): listed per-token price is an unreliable proxy for actual cost (32% of model pairs reverse, up to 28×; thinking-tokens dominate single-turn, turns dominate multi-turn; per-query cost is a high-variance *distribution*, not a number; §2 confirms quality only strengthens the reversal). Relates to **d026** (cost-optimal router that learns) + the **judge-rubric calibration** track. Its own spec → plan → build. **Release:** v1.4.0 has two rc slices in (`1.4.0-rc.2`, untagged) — cutting v1.4.0 is a pending decision. The 7-sprint v2 MVP remains feature-complete + released (v1.3.0). **Parallel optional tracks:** the **active saturation/utilization driver** (d-wa-5 — the headline 99.9%-utilization rank-boost, deferred from Sprint 6); Acceptance-Gate follow-ups (auto-polish loop, cross-exam+chair, **Conductor/job-phase wiring of the gate into a merge/acceptance phase** — the natural next integration, single-reviewer guard, `--diff` guard, brace-balanced JSON scan, solo-discount); Conductor follow-ups / Style-B broker+cockpit; Memory Bridge follow-ups (M1/M2/M3). **Release status — v1.3.0 SHIPPED STABLE 2026-06-21:** Sprints 1–5 + Conductor shipped in **stable v1.2.0** (tag `v1.2.0`); Sprints 6–7 promoted to **stable `v1.3.0`** — plugin `1.3.0-rc.2 → 1.3.0`, release commit `703823c`, annotated tag `v1.3.0` + GitHub release, notes `docs/releases/2026-06-21-v1.3.0.md`. The rc line is closed. No release decision outstanding.
