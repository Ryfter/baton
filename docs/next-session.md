...
**→ NEXT: Sprint 3 — GitHub Projects sync** (import items, normalize labels, update fields; **Labels classify, Project fields decide, Assignees own, Baton routes**). Needs spec → plan → build. Sprint 1's Triage Agent likely feeds the label/field classification. The pre-v2 "next builds" below (cost/speed advisor, autonomous run-loop, MCP ride-along) still stand but are slotted within/after the v2 sprint order.

**Cleanup note:** the Sprint-2 worktree `D:\Dev\baton-usage` was removed cleanly (`git worktree remove`). The older `D:\Dev\baton-triage` leftover was also deleted earlier this session. No stray worktrees remain (`git worktree list` shows only `D:/Dev/Baton`).
