...
1. **Cost-Optimization Engine — Slice C (cascade in the autonomous loop)** OR the **cost/speed advisor** (parked thread 6 below). Slice C = wire `Invoke-CapabilityCascade` into the backlog driver / run-loop, parallel draft fan-out, capacity-surge consumption (the dead `Get-CapacityProfile` assignment in `Invoke-Backlog` gets consumed here). The advisor = two dials (cost × speed) + an infra inventory; fast+cheap corner → recommend *expanding cheap capacity* (2nd $20 platform, add a PC) not deepening expensive; consumes the new `platform` field. Both need their own spec → plan → build. **A→B→C order confirmed; advisor can interleave.** MCP follow-up ride-along whenever convenient: `route-cascade` bridge op + `baton_route` cascade mode (out of Slice B scope by design).
