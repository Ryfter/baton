...
**→ NEXT: a FORK Kevin decides.** (a) **Sprint 5 — Memory Bridge** (Projectmem-style event log + Grimdex promotion; adapter pattern, d047) per the roadmap order. (b) **Conductor follow-ups / Style-B substrate** — the three non-blocking items above, plus the standalone broker + web cockpit behind the file seam, and Conductor Phase 2 (hooks.json + BATON_HOME) / Phase 3 (MCP server). Both need their own spec → plan → build. **Forward hook still standing:** Sprint 6 (Worker Adapter) is where `gh models run <model>` becomes a *budgeted* fleet worker — the mechanism that turns the GitHub allotment into a saturable labor pool the Usage Governor drives toward Kevin's **99.9%-monthly-utilization** target.
