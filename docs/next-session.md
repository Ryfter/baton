...
**⚠ KNOWN ISSUE — judge degraded on the LM Studio setup:** `Get-LlmJudgeGrader` falls back to heuristic with "judge returned no JSON object" — the qwen3.5 models emit thinking preamble the JSON extractor rejects (devstral-on-Ollama judged fine before d043). Until fixed, the ≥0.9 zero-frontier short-circuit can never fire on Firefly. Fix belongs to the **capability-taxonomy work** (Kevin 2026-06-11): judging is a capability claimed by a model good at strict JSON (pin a non-thinking small model for judge duty and/or harden the extractor to strip `<think>`/fences). Verified live: cascade drafts both pinned LM Studio models fine (30b 20s, 9b 13s, box2 graceful fail, $0).
