...
**→ NEXT (Kevin's direction — nothing auto-actionable queued).** The v2 MVP (`v1.3.0`) + the four-slice v1.4 line (`v1.4.0`) are shipped & released; the learned-cost re-rank (slice 5 of the effective-cost track, d060) is now **merged & deployed at plugin `1.4.1-rc.1`**. **0 open issues.** **One decision pending: cut `v1.4.1` stable** — the re-rank is a default-OFF advisory feature, so promoting now vs after real-world `learned_routing: true` validation is Kevin's call (rc line otherwise mirrors the slice-2 → v1.4.0 pattern). Optional next tracks, none auto-actionable, each its own spec→plan→build: (1) realized-cost **`CostResolver`** (token/dollar metering to replace the `estimate` basis — the last "listed price is a lie" gap; would flip `learned_adjust`'s reach from estimate-anchored to measured); (2) **dashboard integration** of the Obsidian Command concepts (Live Fleet Ops first; System-Intelligence MODEL LEADERBOARD consumes the `--json` leaderboard + could surface the live `learned_adjust`/`why` per candidate); (3) acceptance-gate follow-ups (auto-polish loop, task-output bus + mid-DAG gate task, budget-gated reviewer spend); (4) Conductor Style-B broker + cockpit; (5) Memory Bridge M1/M2/M3. **Through-line (Price Reversal Phenomenon, arXiv 2603.23971):** listed per-token price is an unreliable proxy for realized cost — d026's cost-optimal router that learns. Effective cost now measures it (slices 1–2) AND acts on it in routing (slice 3); the remaining honesty gap is the cost *numerator* (estimate → measured), which is the `CostResolver`.
