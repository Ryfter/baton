...
Language profiles (lean): `references/coding-profiles/` — `Invoke-EfficiencyProfileReview` flags bloat. Security recipe: `Get-SecurityRecipe` + `$BATON_HOME/officers/security-scale.json` (hot/warm/cold; never Fable/Sol). Deterministic spine: `Invoke-SecurityScannerSpine`; due orchestration: `Get-SecurityDueProjects` / `Invoke-SecurityDueScans` (Maestro tick via `scripts/maestro-security.ps1`). Operator entry: `scripts/fleet-officers.ps1 -Action scan`. Tests: `scripts/test-officers-lib.ps1` (battery ×5).
