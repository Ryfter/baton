...
- **17 slash commands** invokable from any Claude Code session
- **A live dashboard** at `http://localhost:8765` (start with `python -m dashboard.main` from this repo)
- **A fleet** of paid + local LLM providers (`claude-cli`, `codex`, `gh-copilot`, `gemini-antigravity`, `ollama-local`, `lm-studio` — confirmed via `/fleet doctor`)
...
```
/research "what's the safest way to migrate session tokens without invalidating logins?"
```
...
```
/job-lesson knowledge "session tokens are HS256 today; rotating without invalidating requires a grace window in the validator"
```
...
```
/job-phase next   # research → design (or design → code.sprint-1)
```
...
```
/cost 187.42        # log the new billing total
```
...
```
/kb-index            # incremental — only re-embeds changed files
/kb-search "how do we handle ollama on a second machine"
/consolidate-lessons # promote per-job lessons into the universal KB
/consolidate-decisions # distill decision records + feedback into guidance
```
...
- `/fleet doctor` — re-check every provider's health
