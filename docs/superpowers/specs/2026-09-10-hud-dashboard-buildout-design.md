...
Turn the shipped HUD prototype into **the** Baton dashboard for Kevin's deployment: an always-on, multi-machine, event-sourced activity collector that covers everything the legacy `dashboard/` does, and then retire `dashboard/`.
...
5. At the end, `dashboard/` is deleted from the tree.
...
- **Not** a multi-tenant feature for other Baton plugin users *yet*. But nothing here precludes it: telemetry is opt-in (§8.7), paths are `$BATON_HOME`-relative, and no hostname is hard-coded outside config.
- Not a replacement for `/baton:*` slash commands. The HUD observes the fleet; the commands drive it.
- Not an agent-control console in M1–M6. Write-back is one milestone (M7) and one narrow surface (§7.3).
...
Dependency policy: `psutil` if importable, else stdlib fallback (`os.getloadavg`, `shutil.disk_usage`, `sysctl -n hw.memsize` + `vm_stat` on darwin). The GPU/LM Studio fields overlay `$BATON_HOME/systems/inventory.json` when present.
...
| `POST /config` | `admin` | `{default_frontend, default_theme}` |
| `POST /control/{action}` | `admin` | M7 only. Allowlisted actions (§7.4). Every call also writes a `control` event — the surface audits itself. |
...
### 7.4 Controls (M7, allowlist only)

`ollama/stop-all`, `lmstudio/{load,unload,server-stop}` (ported from `dashboard/routers/controls.py`), and `runs/{id}/answer` (the parked-question reply — the only legacy write path that is part of a workflow). Nothing else. No arbitrary command execution, ever.
...
- Scopes: `ingest` (POST /ingest* only) · `read` (all GETs) · `admin` (POST /config, /control/*).
...
Because a cookie is now ambient, the existing Origin + `Content-Type: application/json` checks on writes become **load-bearing CSRF defence**. They are already implemented (`_require_json_write`, `server.py:88`); they must never be relaxed.
...
`hud/service/dev.baton.hud.plist` → `~/Library/LaunchAgents/`:
...
- `StandardOutPath`/`StandardErrorPath` → `$BATON_HOME/logs/hud.{out,err}.log`, rotated by the pruner (keep 7 × 10 MB)
- Installed by `baton hud install-service`; `baton hud status` reports launchd state + `/healthz`.

Satellite boxes run `dev.baton.hud-forwarder.plist` (macOS) or the platform equivalent — a Windows satellite needs a Scheduled Task, not launchd (open question §15.2).
...
### 8.7 Plugin lifecycle and the multi-tenant guard
...
That would silently turn on telemetry for **every** Baton plugin user. Guard: `hook_emit.py` returns immediately unless `$BATON_HOME/hud-enabled` exists (or `HUD_ENABLED=1`) — an `os.path.exists` check **before** any import, so the disabled path costs ~15 ms. Opt-in, and it is exactly the seam a future multi-tenant mode would build on.
...
**The default theme remains `dark` and remains an open question** (§15.1). The picker ships; `POST /config` gains `default_theme` so the choice is server-side and shared, not per-browser `localStorage`.
...
| `runs.py` | `/partials/runs`, `/partials/assignments`, `/runs/{id}`, `POST /runs/{id}/answer` | `/api/runs`, `/api/runs/{id}`, `POST /control/runs/{id}/answer` | M5 / M7 | the answer POST is the one legacy write path that survives |
...
| `controls.py` | `POST /controls/ollama/stop-all`, `/controls/lmstudio/{load,unload,server/stop}` | `POST /control/{action}` | M7 | admin scope + `control` audit event |
| `kb.py` | `/kb/search`, `/partials/{kb-search,decision}` | **deferred** | — | blocked on open question §15.4 — port at M6, or drop in favour of `/baton:kb-search` (recommended) |
| `maestro.py` (239 loc) | `/maestro/*` incl. `POST /transcribe`, hold/release | **deferred / likely dropped** | — | open question §15.5. STT/voice is out of scope (§16). |
...
1. Every row in §10 is `ported` (gate green) or explicitly `dropped` with Kevin's sign-off on §15.4/§15.5.
2. Two weeks of parallel running with no legacy-only route needed. Honest note: there is no access log on `dashboard/`; this gate is Kevin's judgment, not telemetry. If that is not good enough, add a one-line access log at M4 and measure.
...
Merge `hud-prototype-grok` to `master`. launchd unit on droid. `baton hud {serve,status,install-service,rebuild}`. Migrations framework + `recv_ts`/`event_uid` columns. Retention pruner + nightly backup. `Last-Event-ID` resume. Extended `/healthz`.
**Exit:** survives a reboot unattended; `/healthz` green; a live stream loses zero events across a `kill -9` + restart; DB stops growing without bound; existing 44 tests plus migration/retention/resume tests pass.

### M2 — Multi-machine ingest
`hud/auth.py` token store with scopes/source/machine binding. `POST /ingest/batch`. `hud/forwarder.py` + spool + drain + satellite service unit. TrustedHost extended to the tailnet name. Skew detection. Bare `/api/machines`.
**Exit:** a Claude Code session on a second box appears in the HUD within 2 s; the collector down for 60 s loses nothing on recovery; an `ingest`-scoped token gets 401 on `/events` and 403 on a forged `source`/`machine`.
...
### M5 — Economics, projects, jobs, runs
`/api/{projects,gauges,jobs,runs}`, `rollups_hourly`, `api-rates.json` moved. Dark-factory panel.
**Exit:** parity gates vs `gauges`, `claude_quota`, `project_economics`, `projects`, `ensembles`, `jobs`, `runs`, `dark_factory`. A 5 h burn number matches the legacy gauges page within rounding.

### M6 — Harness adapters, agents, design pass
Codex / grok / cursor-agent / opencode adapters. `/api/agents`, `/api/alerts`. Shared `hud.js`. `obsidian` theme. `v5` gallery snapshot. Default-theme decision landed.
...
### M7 — Controls, plugin wiring, retire `dashboard/`
`POST /control/{action}` (allowlist + audit events), run-answer write-back. Hooks into `hooks/hooks.json` with the opt-in guard. Port cutover. `dashboard/` deleted, design docs moved, all docs updated.
**Exit:** `dashboard/` is gone; no §10 row unaccounted for; the retire checklist (§11.3) is fully ticked; a fresh Baton plugin install with `hud-enabled` absent emits nothing and costs < 20 ms per hook.
...
1. **Default theme.** Still unanswered from the prototype. `dark` today; `obsidian` (the Flight Deck design language) arrives at M6. Pick then, or pick now?
2. **The second machine.** This spec assumes droid is the collector. What is the satellite's hostname, and is it Windows? A Windows satellite needs a Scheduled Task unit instead of launchd, and the machine sampler needs a Windows branch — both cheap, but they belong in M2's scope if the answer is yes.
3. **Tailnet TLS.** `tailscale serve` with a real `*.ts.net` cert, or plain HTTP restricted to the tailnet interface? Recommendation: `tailscale serve`, because the cookie in §8.3 wants `Secure`.
4. **KB router.** Port `dashboard/routers/kb.py` (KB search + decision detail) into the HUD, or drop it and use `/baton:kb-search`? Dropping is my recommendation — it is a search box, not a monitor.
5. **Maestro (239 loc) and STT/voice.** The single biggest legacy router. Is it live and used? If yes it needs its own milestone; if no it is dropped at M7 and this spec assumes dropped.
6. **`mydashboard` router** — reads `~/Dev/MyDashboard`. Confirm droppable.
7. **Retention.** 30 days raw / roll-ups forever — right, or do you want more raw history for debugging?
8. **Controls.** Should the HUD stay read-only forever? M7 ports the ollama/LM Studio controls plus the run-answer write-back; that is the largest single increase in blast radius in the whole plan. Saying "read-only forever" removes it and simplifies §8.2 materially.
9. **Does `hud/` ship to other Baton users at all in v1.x**, or stay Kevin-local until a real multi-tenant story exists? D12 makes shipping safe, but not shipping is also fine and cheaper.
10. **The two-week parallel-running gate (§11.3 step 2)** has no telemetry behind it. Accept it as your judgment call, or add an access log to `dashboard/` at M4 so the decision has data?
...
- **Writing back to agents** beyond the M7 allowlist. No prompt injection into a running session, no arbitrary command execution, ever.
- **WAN exposure**, port forwarding, self-signed certs, any auth provider.
- **Multi-tenant / hosted mode.** Designed-for (D12), not built.
